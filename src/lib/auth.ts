...
export const signupSchema = z.object({
  ...
  if (data.role === 'usuario') {
    if (!data.localidad || data.localidad.trim() === '') ctx.addIssue({ code: 'custom', message: 'La localidad es obligatoria', path: ['locality'] });
    if (!data.direccion || data.direccion.trim() === '') ctx.addIssue({ code: 'custom', message: 'La dirección es obligatoria', path: ['address'] });
  }
});
...
export async function createUser(data: z.infer<typeof signupSchema>) {
  ...
  const user = await prisma.usuario.create({
    data: {
      ...
      nombre: data.name,
      role,
      localidad: data.role === 'usuario' ? data.localidad : null,
      direccion: data.role === 'usuario' ? data.direccion : null,
